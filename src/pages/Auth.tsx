...
export default function Auth() {
  ...
  const validateEmail = (value: string): boolean => {
    ...
    if (result.success) { setEmailError(''); return true; }
  ...
  const validatePassword = (value: string): boolean => {
    ...
    if (result.success) { setPasswordError(''); return true; }
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) {
      toast({ title: "Error signing in with Google", description: error.message, variant: "destructive" });
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md bg-card shadow-lg border-border">
        <CardHeader className="text-center space-y-2">
          ...
          <CardTitle className="text-2xl font-bold text-card-foreground">Trading Journal</CardTitle>
          <CardDescription className="text-muted-foreground">Track your trades, improve your performance</CardDescription>
        </CardHeader>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input id="confirm-password" type={showConfirmPassword ? 'text' : 'password'} placeholder="••••••••"
                      value={confirmPassword} onChange={(e) => handleConfirmPasswordChange(e.target.value)} required
